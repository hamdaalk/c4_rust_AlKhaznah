...
impl Compiler {
    ...
    pub fn emit_operand(&mut self, v: i64) {
        self.code.push(v);
    }
    pub fn emit_syscall(&mut self, syscall_num: i64) {
...
pub fn stmt(&mut self) -> Result<(), String> {
...
pub fn run(&mut self) {
    ...
    pub fn set_source(&mut self, src: &str) {
        self.source = src.chars().collect();
    }

    pub fn set_pc_to_main(&mut self) -> Result<(), String> {
        if let Some(main) = self.symbols.get("main") {
            self.pc = main.value as usize;
            Ok(())
        } else {
            Err("Symbol 'main' not found".into())
        }
    }
